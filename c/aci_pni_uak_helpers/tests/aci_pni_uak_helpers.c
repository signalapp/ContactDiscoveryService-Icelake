...
int test_output_triples_all_pairs_correct()
{
    ...
    create_e164_pni_aci_triples(index, num_records, e164s, response_copy, false, out_triples);
...
int test_output_triples_return_acis_without_uaks()
{
    const char *desc = "compute output triples when all aci-uak pairs are incorrect";
    printf("test: %s\n", desc);
    int retval = 0;
    ...
    create_e164_pni_aci_triples(index, num_records, e164s, response_copy, true, out_triples);
...
int main()
{
    ...
    RUN_TEST(test_output_triples_return_acis_without_uaks());
    return 0;
}
